...
# This function ensures reproducibility by fixing random seeds for Python, NumPy, and PyTorch.
def setup_seed(seed):
    random.seed(seed)
...
# Configures logging to record training progress
...
#loop data - move data to gpu - foward input predication - loss - backward -weight - evaluate metric -learning rate
...
# evaluate model performance on atest dataset - compute loss - calculate correlation cofficients(spearman,pearson)
...
# evaluate trained model on test dataset- calculate performance metrics (SRCC, PLCC, KRCC, RMSE)- compute final predictions(AVG) and save it CSV file
...
#cutom edit
def evaluate_custom_image(image_path, model, tokenizer, image_processor, args):
    """
    Evaluates a custom image using the trained mPLUG-OWL2 model.
    ...
    conv_mode = "mplug_owl2"  # Ensure this mode exists in conv_templates
    ...
    with torch.inference_mode():
        output_tensors = model(input_ids, images=image_tensor, output_hidden_states=True)['hidden_states'][-1]
        output_tensors = torch.mean(output_tensors, dim=1, keepdim=True)  # Aggregate feature representation

    # Print or save the results
    print("Feature Tensor for Custom Image:", output_tensors.shape)
    torch.save(output_tensors, "custom_image_output.pt")
...
# ✔ Trains and evaluates the model, saving the best checkpoint.
# ✔ Runs final testing and logs performance metrics.
...
if __name__ == '__main__':
    ...
    if args.custom_image_path:
        evaluate_custom_image(args.custom_image_path, model, tokenizer, image_processor, args)
        return  # Stop execution after evaluating the custom image
